...
class OrderServiceTest {
    ...
    void testProductsOverAmountPrice() {
        Assertions.assertEquals(10, ordersService.productsOverAmountPrice(1800).size());
    }

    @Test
    void testSumBetweenDates() {
        Assertions.assertEquals(9600, ordersService.sumBetweenDates(LocalDate.of(2021, 5, 31), LocalDate.of(2021, 6, 2)));
    }

    @Test
    void testFindByName() {
        Assertions.assertEquals(product, ordersService.findByName("Phone"));
    }

    @Test
    void testFindByNameNoSuchProduct() {
        Exception ex = assertThrows(IllegalArgumentException.class, () -> ordersService.findByName("Számítógép"));
        Assertions.assertEquals("No such product.", ex.getMessage());
    }

    @Test
    void testFindOrderWithMostExpensiveProduct() {
        Assertions.assertEquals(order, ordersService.findOrderWithMostExpensiveProduct());
    }
}